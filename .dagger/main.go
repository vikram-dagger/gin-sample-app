...
func OpenPR(
	...
	_, err = dag.Container().
		...
		WithExec([]string{"git", "commit", "-m", fmt.Sprintf("Follow-up to PR #%s", prNumber)}).
	...
	newPR := &github.NewPullRequest{
		...
		Body:  github.String(fmt.Sprintf("This PR follows-up on PR #%s using `%s`.", prNumber, newBranch)),
	}
